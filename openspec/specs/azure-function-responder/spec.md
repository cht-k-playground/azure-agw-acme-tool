# azure-function-responder Specification

## Purpose
Azure Functions HTTP trigger that serves ACME HTTP-01 challenge responses. The function reads the `ACME_CHALLENGE_RESPONSE` App Setting (written by the CLI before triggering ACME CA validation) and returns it as `text/plain` at the `/.well-known/acme-challenge/{token}` route.
